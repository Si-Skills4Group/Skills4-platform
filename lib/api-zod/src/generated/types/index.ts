/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * Employer Engagement CRM API — Skills4CRM
 * OpenAPI spec version: 0.2.0
 */

export * from "./activityEventType";
export * from "./activityLogEntry";
export * from "./activityLogEntryMetadata";
export * from "./callOutcome";
export * from "./contact";
export * from "./createContactRequest";
export * from "./createEngagementRequest";
export * from "./createOrganisationRequest";
export * from "./createTaskRequest";
export * from "./dashboardSummary";
export * from "./dashboardSummaryEngagementsByStageItem";
export * from "./dashboardSummaryOrganisationsByTypeItem";
export * from "./dashboardSummaryTasksByStatusItem";
export * from "./engagement";
export * from "./engagementStage";
export * from "./engagementStatus";
export * from "./engagementType";
export * from "./handoverRequest";
export * from "./handoverResult";
export * from "./handoverStatus";
export * from "./healthStatus";
export * from "./listActivityParams";
export * from "./listContactsParams";
export * from "./listEngagementsParams";
export * from "./listOrganisationsParams";
export * from "./listTasksParams";
export * from "./logCallRequest";
export * from "./loginRequest";
export * from "./loginResponse";
export * from "./organisation";
export * from "./organisationStatus";
export * from "./organisationType";
export * from "./outreachChannel";
export * from "./preferredContactMethod";
export * from "./sdrDashboardSummary";
export * from "./sdrDashboardSummaryConversionFunnelItem";
export * from "./sdrDashboardSummaryProspectsByStageItem";
export * from "./sdrManagerReport";
export * from "./sdrManagerReportMeetingsByWeekItem";
export * from "./sdrManagerReportOverdueFollowUpsItem";
export * from "./sdrManagerReportRepPerformanceItem";
export * from "./sdrManagerReportTerminalStageDistributionItem";
export * from "./sdrStage";
export * from "./task";
export * from "./taskPriority";
export * from "./taskStatus";
export * from "./updateContactRequest";
export * from "./updateEngagementRequest";
export * from "./updateOrganisationRequest";
export * from "./updateTaskRequest";
export * from "./updateUserRequest";
export * from "./user";
export * from "./userRole";
