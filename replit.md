...
- **SDR Queue (`/sdr`):** Call-led Apollo/Salesloft-style SDR workspace. Features: (1) FunnelBar — horizontal scrollable stage-count strip; (2) FilterPanel — collapsible sidebar with owner/lead-source/call-outcome dropdowns and quick-filter checkboxes (follow-up required, contact made, overdue, meeting booked, handover pending); (3) compact 6-column table — Contact/Org | Stage badge | Last Call Outcome badge | Owner initials | Next date (red if overdue) | Call count pill; (4) ProspectDrawer — right-slide panel with "Log Call" dropdown (quick: No Answer/Voicemail/Gatekeeper/Wrong Person; detailed: Spoke–*, Meeting Booked), Email/LinkedIn quick-log, stage action row, info grid with call stats; modals: Log Call (pre-set outcome, follow-up date, note), Meeting, Disqualify, Create Task, Change Stage, Handover/Qualify. POST `/api/engagements/:id/log-call` auto-derives stage from outcome and increments call/touch counts. Warm callback workflow: `spoke_call_back_later` outcome shows amber "Callback" chip in queue row and "Callback Requested" card in drawer; `spoke_send_info` shows cyan "Send info" chip and "Send Info Before Calling Back" drawer card. LogCallModal shows outcome-specific guidance banners, quick date presets (Tomorrow/2d/3d/1w/2w), auto-fills default date, and renames the submit button per warm outcome.
- **SDR Manager Report (`/sdr-report`):** Manager-facing reporting view. Sections: (1) Conversion by Rep table — per-rep stats (total prospects, calls dialled, contact made, meetings booked, qualified leads, conversion %, overdue follow-ups) with coloured avatar, mini progress bar, and colour-coded stat pills; (2) Meetings Booked by Week — 8-week bar chart of scheduled meeting dates; (3) Disqualification Reasons — horizontal bar chart of terminal stage distribution (nurture, unresponsive, do_not_contact, bad_data, changed_job, disqualified); (4) Pipeline Progress by Rep — grouped horizontal bar chart of meetings + qualified per rep; (5) Overdue Follow-ups — table of overdue prospects with rep, days overdue badge, last call outcome badge, follow-up reason. Data from GET `/api/dashboard/sdr/manager` endpoint.
- **Settings (`/settings`):** User profile settings.

**SDR Data Model Extension:**
The `Engagement` entity is extended with SDR and call-tracking fields including `engagement_type`, `sdr_stage`, `qualification_status`, `lead_source`, `sdr_owner_user_id`, and 14 call-tracking columns: `callAttemptCount`, `lastCallDate`, `nextCallDate`, `lastCallOutcome`, `contactMade`, `voicemailLeft`, `followUpRequired`, `followUpReason`, `mqlStatus`, `sqlStatus`, `opportunityCreated`, `latestNote`, `pitchDeckSent`, `infoSentDate`.
SDR stage values (active): `new`, `researching`, `attempted_call`, `contact_made`, `no_contact`, `follow_up_required`, `replied`, `interested`, `meeting_booked`, `qualified`. Terminal: `nurture`, `unresponsive`, `do_not_contact`, `bad_data`, `changed_job`, `disqualified`. Legacy (backward-compat): `outreach_started`, `contacted`, `response_received`. Terminal stages auto-set `status = closed_lost`. `disqualified` requires `disqualificationReason`.
Call outcomes (CallOutcome enum): `no_answer`, `voicemail_left`, `gatekeeper`, `wrong_person`, `spoke_call_back_later`, `spoke_send_info`, `spoke_not_interested`, `spoke_interested`, `meeting_booked`.
SDR component library: `artifacts/crm/src/components/sdr/` — `constants.ts`, `FunnelBar.tsx`, `FilterPanel.tsx`, `ProspectDrawer.tsx`, `index.ts`.
